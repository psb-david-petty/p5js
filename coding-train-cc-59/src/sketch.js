...
function fontData(message, w=width, h=height, factor=0.8125, pt=100) {
  let
    xs = new Array(), ys = new Array(),
    points = font.textToPoints(message, 0, h, pt);
  for (const point of points) {
    xs.push(point.x);
    ys.push(point.y);
  }
  let x = max(...xs), y = h - min(...ys);
  ...
  console.log(`'${message}' in ${pt}pt is ${int(x)} x ${int(y)} `
...
function setup() {
  ...
  message = msg ? msg.replace(/\+/g, ' ') :
    sketch ? sketch.elt.title : message;
